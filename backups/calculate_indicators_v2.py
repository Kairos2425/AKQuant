...
def generate_analysis(df, summary, resistance, support):
    ...
    analysis["kline_ma"] = {
        ...
        "methodology": [
            "均线（MA）是将一定时期内证券价格加以平均，连成曲线用以观察价格趋势的技术指标。短期均线（MA5/MA10）反映短线资金动向，中期均线（MA20）是波段操作的重要参考，长期均线（MA60）代表中期趋势方向。",
            "均线多头排列（短期在上、长期在下）是上升趋势的确认信号；空头排列（长期在上、短期在下）则预示下降趋势。均线交叉（金叉/死叉）是趋势转折的早期信号，但存在滞后性，需结合其他指标确认。",
            "ATR（Average True Range）衡量价格波动幅度而非方向。ATR越大，日内波动越剧烈，止损位应相应放宽；ATR缩小则预示可能的变盘。",
        ],
        "interpretation": {
            "多头排列": "MA5>MA10>MA20>MA60，且价格在MA5上方 → 强势上升，顺势做多",
            "金叉信号": "短期均线上穿长期均线 → 趋势可能由空转多，买入参考信号",
            "死叉信号": "短期均线下穿长期均线 → 趋势可能由多转空，卖出参考信号",
            "价格与均线关系": "价格高于均线→均线构成支撑；价格低于均线→均线构成压力",
            "ATR应用": f"当前ATR={atr_pct:.1f}%，作为止损参考：短线止损可设在入场价下方1-2倍ATR处",
        },
    }
    ...
    analysis["macd"] = {
        ...
        "methodology": [
            "MACD（指数平滑异同移动平均线）由Gerald Appel发明，通过快慢均线的差值（DIF）及其移动平均（DEA）来研判趋势。DIF=EMA(12)-EMA(26)，DEA=EMA(DIF,9)，柱状图=(DIF-DEA)×2。",
            "DIF与DEA在零轴上方代表中期多头市场，下方代表中期空头市场。零轴是牛熊分界线，穿越零轴信号比金叉/死叉更可靠但更滞后。",
            "MACD柱状图反映DIF与DEA的距离变化。柱体增长→动能增强；柱体缩短→动能减弱。柱体高度变化比颜色变化更具参考价值。",
        ],
        "interpretation": {
            "金叉买入": "DIF从下方上穿DEA → 短期买入信号，结合零轴位置判断强弱（零轴上方的金叉信号更可靠）",
            "死叉卖出": "DIF从上方下穿DEA → 短期卖出信号，零轴下方的死叉确认下跌趋势延续",
            "零轴穿越": "DIF向上突破零轴→进入多头市场；向下跌破零轴→进入空头市场",
            "顶背离": "价格创新高但MACD高点下降 → 上涨动力衰竭，高位反转预警",
            "底背离": "价格创新低但MACD低点抬升 → 下跌动力衰竭，低位反转信号",
            "柱状图解读": f"当前柱高{abs(hist):.4f}，{'持续放大中，趋势加速' if abs(hist) > abs(prev_hist) else '逐步缩短，趋势减速'}",
        },
    }
    ...
    analysis["rsi"] = {
        ...
        "methodology": [
            "RSI（Relative Strength Index，相对强弱指标）由Welles Wilder于1978年提出，衡量一定时期内价格上涨幅度与下跌幅度的比率，反映多空力量对比。RSI=100-[100/(1+RS)]，其中RS=平均上涨幅度/平均下跌幅度。",
            "RSI理论区间为0-100。传统用法：>70为超买（价格可能回调），<30为超卖（价格可能反弹）。在强趋势市场中，RSI可长时间维持在超买/超卖区间，因此需结合趋势方向使用。",
            "RSI背离是重要的反转预警：价格与RSI走势方向相反时，往往预示趋势即将逆转。顶背离（价格新高RSI未新高）→卖出信号；底背离（价格新低RSI未新低）→买入信号。",
        ],
        "interpretation": {
            "超买区间(>70)": "价格短期涨幅过大，技术上有回调需求。强牛市中可能持续超买，不宜单独作为卖出依据",
            "超卖区间(<30)": "价格短期跌幅过大，技术上有反弹需求。弱熊市中可能持续超卖，需等待确认信号",
            "50中轴": "RSI>50代表多头占优，RSI<50代表空头占优。50附近震荡代表多空均衡",
            "RSI趋势线": "连接RSI的高低点形成的趋势线突破，往往领先于价格趋势线突破",
            "背离识别": "顶背离至少需要两个依次降低的RSI高点+两个依次抬高的价格高点；底背离反之",
        },
    }
    ...
    analysis["kdj"] = {
        ...
        "methodology": [
            "KDJ（随机指标）由George Lane提出，通过比较收盘价在近期价格区间中的位置来衡量价格动量。核心思想：上升趋势中收盘价倾向于接近最高价，下降趋势中收盘价倾向于接近最低价。",
            "计算过程：先计算RSV（未成熟随机值）= (收盘价-N日最低)/(N日最高-N日最低)×100；K=RSV的M1日移动平均；D=K的M2日移动平均；J=3K-2D。J值波动最为剧烈，常用于提前预判。",
            "KDJ反应比MACD更灵敏，适合短线交易。但在横盘震荡中容易产生虚假信号（钝化），需结合趋势指标过滤。",
        ],
        "interpretation": {
            "K线与D线交叉": "K上穿D（金叉）→短线买入；K下穿D（死叉）→短线卖出。20以下的金叉和80以上的死叉信号更可靠",
            "J值极值": "J>100 → 严重超买，短线见顶概率大；J<0 → 严重超卖，短线见底概率大",
            "钝化现象": "强趋势中K/D/J可能长时间在超买/超卖区钝化，此时交叉信号失效，需等待指标脱离极端区后再操作",
            "KDJ与价格背离": "价格新高而KDJ未新高→顶背离；价格新低而KDJ未新低→底背离。背离后交叉确认信号更可靠",
        },
    }
    ...
    analysis["bollinger"] = {
        ...
        "methodology": [
            "布林带（Bollinger Bands）由John Bollinger于1980年代提出，由三条轨道组成：中轨=MA(20)，上轨=中轨+2倍标准差，下轨=中轨-2倍标准差。基于统计学原理，价格约95%的时间在上下轨之间运行。",
            "带宽（Band Width）反映市场波动率：带宽收窄（Squeeze）→波动率压缩，预示大幅突破行情临近；带宽扩张→波动率放大，趋势行情正在展开。",
            "布林带的核心交易策略不是简单的「碰到上轨卖、碰到下轨买」，而是结合带宽变化和价格相对位置判断趋势强度和转折信号。",
        ],
        "interpretation": {
            "价格触及上轨": "强势上涨中可沿上轨运行（Band Riding），但需警惕回归中轨的引力。上轨附近+RSI超买→回调概率增大",
            "价格触及下轨": "弱势下跌中可沿下轨运行，但超跌后往往有技术反弹。下轨附近+RSI超卖→反弹概率增大",
            "带宽收窄(Squeeze)": f"当前带宽{bb_width:.1f}%，10日前{bb_width_10d:.1f}%——{'波动压缩，突破在即' if bb_width < bb_width_10d * 0.8 else '波动正常'}",
            "中轨方向": f"中轨（MA20）{'向上运行，中期趋势偏多' if bb_mid > df['BOLL_MID'].iloc[-6] else '向下运行，中期趋势偏空'}",
        },
    }
    ...
    analysis["volume"] = {
        ...
        "methodology": [
            "成交量是技术分析中唯一不可操纵的核心指标，反映市场参与者的真实意愿。\"量在价先\"是技术分析的重要原则——成交量变化往往领先于价格变化。",
            "量价关系基本法则：量增价升（健康上涨）→多头趋势确认；量增价跌（恐慌抛售）→空头趋势加速；量缩价升（上涨乏力）→警惕见顶；量缩价跌（抛压减轻）→关注见底。",
            "换手率反映股票流动性：科创板股票换手率通常高于主板，5%以上为活跃。结合量比（当日成交量/20日均量）判断当日成交相对强度。",
        ],
        "interpretation": {
            "放量上涨": "成交量显著放大+价格上涨 → 增量资金入场，上涨可持续性较强。关注是否持续放量，突然缩量则需警惕",
            "放量下跌": "成交量显著放大+价格下跌 → 恐慌盘涌出或主力出货。次日能否止跌企稳是关键",
            "缩量上涨": "成交量萎缩+价格上涨 → 惜售或无量空涨。缺乏量能支撑的上涨往往难以持续",
            "缩量下跌": "成交量萎缩+价格下跌 → 抛压减轻，可能接近底部。但需等待放量阳线确认反转",
            "天量天价": f"近60日最大成交量出现在{vol_max_date}，该日的量价表现对判断后续走势具有重要参考意义",
        },
    }
    ...
    analysis["sr"] = {
        ...
        "methodology": [
            "支撑位和阻力位是技术分析的基础概念。支撑位是价格下跌过程中遇到买盘集中从而止跌反弹的价格区域；阻力位是价格上涨过程中遇到卖盘集中从而遇阻回落的价格区域。",
            "本系统通过局部极值聚类算法自动识别S/R位：在20日窗口中寻找局部最高点（潜在阻力）和局部最低点（潜在支撑），将距离在3%以内的多个极值点合并为一个有效水平。",
            "风险收益比（Reward/Risk Ratio）是仓位管理的核心指标。一般要求R/R>2:1才值得参与，即预期收益至少是潜在亏损的2倍。止损应设在最近支撑位下方，目标价设在最近阻力位附近。",
        ],
        "interpretation": {
            "阻力位突破": "价格放量突破阻力位→该阻力转化为新支撑位，可顺势做多。缩量接近阻力→大概率遇阻回落",
            "支撑位防守": "价格回踩支撑位不破→买入良机。放量跌破支撑→止损离场，支撑转化为新阻力",
            "S/R互换": "被突破的阻力位变为支撑位，被跌破的支撑位变为阻力位——这是技术分析中最可靠的原理之一",
            "风险收益比应用": f"假设在¥{current_price:.2f}入场，止损设在¥{nearest_s:.2f}（-{downside:.1f}%），目标价¥{nearest_r:.2f}（+{upside:.1f}%），{'该交易盈亏比合理' if rr_ratio >= 2 else '该交易盈亏比不足，建议等待更好的入场点'}",
        },
    }
    ...
    analysis["comprehensive"] = {
        ...
        "methodology": [
            "多因子综合研判系统采用加权评分法，将均线趋势、MACD、RSI、成交量、KDJ、布林带、支撑阻力共7大维度8个因子的信号量化为统一分数（0-100分），避免单一指标的片面性。",
            "各因子权重根据历史回测有效性分配：均线趋势（权重2）和MACD（权重2）为核心趋势指标；成交量（权重±2）验证趋势可信度；RSI/KDJ/布林带/支撑阻力（各权重±1）为辅助确认指标。",
            "评分规则：>65分为偏多，40-65分为中性震荡，<40分为偏空。所有技术指标均存在滞后性，综合研判的结果应作为决策参考而非唯一依据。",
        ],
        "interpretation": {
            "高分(>65)": "多指标共振看多，适合趋势跟随策略。注意：高分不意味着没有回调风险，需设好止损",
            "中分(40-65)": "多空力量均衡，适合区间交易或观望。震荡市中趋势策略容易反复止损",
            "低分(<40)": "空头信号主导，适合防御策略。低分环境下逆势做多风险极高",
            "信号共振": "多个同向信号的可靠性远超单一信号。当趋势指标+震荡指标+量能指标同时指向同一方向时，判断的可信度最高",
        },
        "key_points": [
